class Conversation < ActiveRecord::Base
  ...
  # either returns a conversation or creates one if it doesn't exist
  # NOTE: A conversation can only be created by a renter
  def self.create_or_get_conversation(params, current_user)
    ...
    if listing
      ...
      if current_user.id == listing.user_id
        ...
        # NOTE: this blank check is only done here since only the renter can initiate a message
  ...
  # update conversation to show message was read
  def read
