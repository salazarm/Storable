class Listing < ActiveRecord::Base
    ...
    # overridden to include referenced classes in the json representation
    def as_json(options={})
      super(:include =>[:images, :location, :reserved_dates, :transaction_reviews])
    end

    # create a reserved date associated with the transaction
    ...
    # Returns true if this current user can still review this listing. Since a user can book a 
    #   listing multiple times we check if (number of transactions) - (number of reviews) >= 1. 
    #   This implies that the user can still review this listing.
    ...
    # searches through listings based on the filters passed in
